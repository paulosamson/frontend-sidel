...
(function () {
    var u = $(".article-content"),
        v = u.data("disable-lightbox");
    if (v === 1) {
        return
    }
    ...
    function FnIsValidImageUrlPngToJpeg(url) {
        $.ajax({
            url: url,
            type: 'HEAD',
            async: false,
    ...
    function FnIsValidImageUrl(url) {
        $.ajax({
            url: url,
            type: 'HEAD',
            async: false,
            error: function () {
                url = FnIsValidImageUrlPngToJpeg(url.replace(".jpg", ".png"));
            },
            success: function () {
                return;
            }
    ...
    function FnGetImageUrl(href, width, height) {
        //var imageUrl = FnIsValidImageUrl(href.replace("_" + width + "x" + height, ""));
        return href.replace("_" + width + "x" + height, "");
    }
    ...
    $(".article-content").on("click", "img", function () {
        if ($(this).data("disable-lightbox") === 1)
            return;

        var w = t(this);
        var ancestor = $('.article-content');


        var enableGallery = ancestor.data("enable-gallery") === 1;
        if (enableGallery) {
            var galleryAfter = [];
            var galleryBefore = [];
            var images = ancestor.find("img");
            var imageUrl = FnGetImageUrl(this.src, this.width, this.height);
            var current = this;
            ...
            images.each(function () {
                if (this === current || isCurrentAdded) {
                    galleryAfter.push({
                        href: FnGetImageUrl(this.src, this.width, this.height),
                        title: this.alt
                    });
                    isCurrentAdded = true;
                }
            ...
            var gallery = galleryAfter.concat(galleryBefore);
            ...
            /* Single Image */
            var x = $("<img/>").load(function () {
                $.fancybox.open([w], {
                    padding: 10
                })
            }).error(function () {
                w.href = w.hrefPNG;
                var y = $("<img/>").load(function () {
                    $.fancybox.open([w], {
                        padding: 10
                    })
                }).error(function () {
                    var z = $("<img/>").load(function () {
                        $.fancybox.open([w], {
                            padding: 10
                        })
                    }).error(function () { }).attr("src", w.href)
                }).attr("src", w.href)
            }).attr("src", w.href)
        }
